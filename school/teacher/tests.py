...
class TeacherTests(BaseAPITest):
    ...
    def test_deleting_teacher_with_creds(self):
        resp = self.create_teacher(first_name="TeTeacher", streams=[1], phone="986", school=2)
        print(resp.json())
        teacher_id = 3
    ...
    def test_deleting_school_wwith_teacher(self):
        ...
        resp = self.auth_client.get(reverse("retrieve_update_destroy_school", kwargs={"pk": school}))
        self.assertEquals(resp.status_code, status.HTTP_200_OK)
        resp = self.auth_client.delete(reverse("retrieve_update_destroy_school", kwargs={"pk": school}))
