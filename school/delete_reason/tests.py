...
class DeleteReasonTests(BaseAPITest):
    ...
    def test_retrieving_delete_reason(self):
        ...
        resp = self.client.get(reverse("retrieve_update_destroy_delete_reason", kwargs={"pk": 1}))
        self.assertEquals(resp.status_code, status.HTTP_200_OK)
        self.assertEquals(resp.data["name"], "TheDeleteReason")

    def test_updating_delete_reason(self):
        resp = self.client.patch(reverse("retrieve_update_destroy_delete_reason", kwargs={"pk": 1}), data={"name": "Hello"})
        self.assertEquals(resp.status_code, status.HTTP_200_OK)
    ...
    def test_students_delete_reason_stats(self):
        ...
        url = reverse("list_dynamic_student_absent_reasons_statistics", kwargs={"type": "student"})
        resp = self.auth_client.get(url)
        # print(resp.json())
        self.assertEquals(resp.status_code, 200)

        url = reverse("list_dynamic_student_absent_reasons_statistics", kwargs={"type": "class"})
        resp = self.auth_client.get(url)
        # print(resp.json())
        self.assertEquals(resp.status_code, 200)

        url = reverse("list_dynamic_student_absent_reasons_statistics", kwargs={"type": "stream"})
        resp = self.auth_client.get(url)
        # print(resp.json())
        self.assertEquals(resp.status_code, 200)

        url = reverse("list_dynamic_student_absent_reasons_statistics", kwargs={"type": "school"})
        resp = self.auth_client.get(url)
        print(resp.json())
        self.assertEquals(resp.status_code, 200)
