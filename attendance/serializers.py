...
from attendance.common import attendance_taken_signal
...
class StreamAttendanceSerializer(serializers.Serializer):
    ...
    def send_attendance(self, date, stream):
        res = Attendance.objects.filter(date=date, stream_id=stream).aggregate(
            present_count=Count("id", filter=Q(status="1")),
            absent_count=Count("id", filter=Q(status="0")),
        )
        attendance_taken_signal.send(
