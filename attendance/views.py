...
from mylib.my_common import MyDjangoFilterBackend, MyStandardPagination, str2bool, MyCustomException
from attendance.models import ATTENDANCE_STATS_DEFAULT_FIELDS, ATTENDANCE_STATS_DEFINTIONS, Attendance, AttendanceHistory
from attendance.serializers import AttendanceSerializer, TakeAttendanceSerializer, SerializerAllPercentages, SerializerAll, StreamAttendanceSerializer
...
from mylib.my_common import FilterBasedOnRole
...
class ListAttendanceDynamicStatisticsAPIView(MyCustomDyamicStats, generics.ListAPIView):
    ...
    serializer_class = BaseDynamicStatsSerializer
    queryset = Attendance.objects.annotate(gender=F("student__gender")).select_related("student")
    filter_backends = (MyDjangoFilterBackend,)
    permission_classes = (IsAuthenticated,)
    stat_type = ""
    filter_mixin = AttendanceFilter
...
class ListCreateAttendanceStatistics(generics.ListAPIView):
    ...
    pagination_class = MyStandardPagination
    nonefomats = ["yearly", "class", "monthly", "village", "school", "gender", "county", "oosc", "partner", "school"]
    fakepaginate = False
    ...
    def resp_fields(self):
        pm = Count(
            Case(
                When(Q(student__gender="M") & Q(status=1) & Q(student__active=True), then=1),
                output_field=IntegerField(),
            )
        )
        pf = Count(
            Case(
                When(Q(student__gender="F") & Q(status=1) & Q(student__active=True), then=1),
                output_field=IntegerField(),
            )
        )
        af = Count(
            Case(
                When(Q(student__gender="F") & Q(status=0) & Q(student__active=True), then=1),
                output_field=IntegerField(),
            )
        )
        am = Count(
            Case(
                When(Q(student__gender="M") & Q(status=0) & Q(student__active=True), then=1),
                output_field=IntegerField(),
            )
        )
        return pm, pf, af, am
    ...
    def get_format(self, format):
        ...
        if format == "monthly":
            ...
            return Concat("stream__base_class", Value("_"), F("stream__name"), Value("_"), F("stream__id"), output_field=CharField())
        elif format == "class":
        ...
        #     return Concat("student__is_oosc",Value(''),output_field=BooleanField())
            ...
            return Concat("stream__school__name", Value("_"), F("stream__school_id"), output_field=CharField())
        else:
            # #print daily
            return daily
