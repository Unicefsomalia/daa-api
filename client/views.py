...
from client.permission import IsSystemAdmin
from client.serializers import (
    DeactiveTeacherAccountSerializer,
    MyUserSerializer,
...
from client.users.serializers import ResetTeacherPasswordSerializer
...
class DeactiveTeacherProfileAccount(APIView):
    serializer_class=DeactiveTeacherAccountSerializer
    permission_classes = [
        IsAuthenticated,
    ]
    ...
    def post(self, request, format=None):
        ...
        if user.teacher is not None:
            if user.teacher.is_non_delete:
                raise MyCustomException("")
            user.teacher.delete()
            return Response({"detail":"Teacher profile deleted successfully"})
        raise MyCustomException("Only personal profiles can be deleted.")


class CreateListUser(generics.ListAPIView):
    ...
    def perform_create(self, serializer):
        serializer.save(
            old_password=serializer.initial_data.get("password"),
...
class ForgotPasswordView(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            MySendEmail(
                "  Somali Digital Attendance Password Reset Code",
                "forgot.html",
                data,
                [user.email],
            )
...
class VerifyAccountView(APIView):
    ...
    def get(self, request, format=None):
        ...
        failed_data = {
            "response": "Token expired . Try resending the verification email",
        ...
        if clients[0].verified:
            return Response(
                {"response": "Account already verified.", "heading": "Account active"},
                template_name="verify_account.html",
            )
        clients[0].verified = True
        clients[0].save()
        return Response(
            {"response": "Account verified.", "heading": "Success"},
            template_name="verify_account.html",
        )
