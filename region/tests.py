...
class RegionTests(BaseAPITest):
    ...
    def test_retrieving_region(self):
        resp = self.auth_client.get(reverse("retrieve_update_destroy_region", kwargs={"pk": 1}))
        # print(resp.json())
        self.assertEquals(resp.status_code, status.HTTP_200_OK)
        self.assertEquals(resp.data["name"], "TheRegion")

    def test_updating_region(self):
        resp = self.auth_client.patch(reverse("retrieve_update_destroy_region", kwargs={"pk": 1}), data={"name": "Hello"})
        self.assertEquals(resp.status_code, status.HTTP_200_OK)
