...
class DistrictTests(BaseAPITest):
    ...
    def test_retrieving_district(self):
        resp = self.auth_client.get(reverse("retrieve_update_destroy_district", kwargs={"pk": 1}))
        self.assertEquals(resp.status_code, status.HTTP_200_OK)
        self.assertEquals(resp.data["name"], "TheDistrict")

    def test_updating_district(self):
        resp = self.auth_client.patch(reverse("retrieve_update_destroy_district", kwargs={"pk": 1}), data={"name": "Hello"})
        self.assertEquals(resp.status_code, status.HTTP_200_OK)
